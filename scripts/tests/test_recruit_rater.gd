...
func run(t) -> void:
	...
	t.assert_eq(qb_stars, 5, "top QB earns 5-star rating")
	t.assert_true(k_stars <= 3, "specialists are capped at 3 stars")

	var sample := players[0] as Dictionary
	t.assert_true(sample.has("composite_score"), "composite score assigned")
	...
	t.assert_true(hi_score > lo_score, "higher percentiles yield higher composite")
