...
const TEST_SCRIPTS := [
	"res://scripts/tests/test_pa6_1_satisfaction_from_playing_time.gd",
	"res://scripts/tests/test_pa6_2_morale_affects_development.gd",
	"res://scripts/tests/test_pa6_3_transfer_decisions.gd"
]
...
func _init() -> void:
	...
	if total_failures.is_empty():
		print("All PA6 (Player Agency) tests passed (%d)." % TEST_SCRIPTS.size())
		quit(0)
		return
