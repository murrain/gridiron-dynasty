## GdUnit4 test suite for H4.6: Championship Drought Tracking
##
...
func test_recent_champion_has_zero_drought() -> void:
	...
	assert_int(drought).is_equal(0)


func test_never_won_has_negative_one() -> void:
	var world_state := _create_test_world()
	var config := _create_test_config()
	var positions_cfg := {"QB": {"core_stats": []}}
	var main_cfg := {"class_rules": {}}
	...
	for team_id in team_history.keys():
		if not champion_ids.has(team_id):
			...
			assert_int(drought).is_equal(-1)
			checked_non_champion = true

	assert_bool(checked_non_champion).is_true()


func test_drought_increases_each_year() -> void:
	...
	assert_int(drought).is_equal(3)


func test_drought_resets_on_championship() -> void:
	...
	assert_int(drought).is_equal(0)
	assert_int(champ_count).is_equal(2)
	assert_bool(2023 in champ_years).is_true()
