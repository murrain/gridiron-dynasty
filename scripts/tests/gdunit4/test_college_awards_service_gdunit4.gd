## GdUnit4 test suite for CollegeAwardsService
##
## Tests college award selection including Heisman Trophy, All-Americans,
## conference awards, and mock draft rankings.
extends GdUnitTestSuite

const CollegeAwardsService = preload("res://scripts/world/CollegeAwardsService.gd")


func test_select_heisman_trophy() -> void:
	...
	assert_bool(result.has("finalists")).is_true()

	var winner: Dictionary = result.get("winner", {})
	assert_bool(winner.is_empty()).is_false()
	assert_bool(winner.get("position") in ["QB", "RB", "WR"]).is_true()

	var finalists: Array = result.get("finalists", [])
	assert_int(finalists.size()).is_less_equal(4)


func test_select_all_americans() -> void:
	var eligible_players := [
	...
	assert_bool(result.has("third_team")).is_true()


func test_select_conference_awards() -> void:
	var eligible_players := [
	...
	assert_bool(result.has("big_ten")).is_true()

	var big_ten: Dictionary = result.get("big_ten", {})
	assert_bool(big_ten.has("offensive_poy")).is_true()
	assert_bool(big_ten.has("defensive_poy")).is_true()


func test_generate_mock_draft_rankings() -> void:
	...
	assert_bool(result.has("source_rankings")).is_true()
	assert_bool(result.has("consensus")).is_true()

	var source_rankings: Dictionary = result.get("source_rankings", {})
	assert_bool(source_rankings.has("espn")).is_true()
	assert_bool(source_rankings.has("pff")).is_true()


func test_hype_vs_talent_gap() -> void:
	var player := {
		"player_id": "overhyped",
	...
	assert_float(gap).is_between(-1.0, 1.0)


func test_hype_vs_talent_gap_unranked_player() -> void:
	var mock_rankings := {
		"consensus": [
			{"player_id": "overhyped", "rank": 5}
		]
	}
