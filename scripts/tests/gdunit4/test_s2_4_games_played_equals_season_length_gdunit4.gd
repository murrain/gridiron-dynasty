## GdUnit4 test suite for S2.4: Games Played Equals Season Length
##
...
func test_college_season_12_games() -> void:
	var world_state := {}
	...
	assert_int(int(player_stats["games_played"])).is_equal(12)

	# Verify accumulated stats are reasonable for 12 games
	assert_int(int(player_stats["pass_attempts"])).is_greater(200)
	assert_int(int(player_stats["pass_yards"])).is_greater(2000)


func test_nfl_season_17_games() -> void:
	var world_state := {}
	...
	assert_int(int(player_stats["games_played"])).is_equal(17)

	# Verify accumulated stats are reasonable for 17 games
	assert_int(int(player_stats["rush_attempts"])).is_greater(200)
	assert_int(int(player_stats["rush_yards"])).is_greater(800)


func test_partial_season_injury_simulation() -> void:
	var world_state := {}
	...
	assert_int(int(player_stats["games_played"])).is_equal(6)

	# Stats should reflect partial season
	assert_int(int(player_stats["receptions"])).is_less(100)


func test_multiple_seasons_games_played() -> void:
	var world_state := {}
	var positions_cfg := {"QB": {"core_stats": []}}
	var main_cfg := {"class_rules": {}}
	var rng := RandomNumberGenerator.new()
	...
	assert_int(int(player_career[2025]["games_played"])).is_equal(12)
	assert_int(int(player_career[2026]["games_played"])).is_equal(12)
	assert_int(int(player_career[2027]["games_played"])).is_equal(12)
	...
	assert_int(total_games).is_equal(36)

	# Each year's stats should be independent
	var attempts_2025: int = int(player_career[2025]["pass_attempts"])
	var attempts_2026: int = int(player_career[2026]["pass_attempts"])
	var attempts_2027: int = int(player_career[2027]["pass_attempts"])
...
func test_roster_changes_games_played() -> void:
	var world_state := {}
	...
	assert_str(String(player_stats["team_id"])).is_equal("team_002")
