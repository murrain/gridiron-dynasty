## GdUnit4 test suite for H4.2: Championship History Accuracy
##
...
func test_champions_have_correct_count() -> void:
...
func test_championship_years_match_championships() -> void:
	var world_state := _create_test_world()
	var config := _create_test_config()
	var positions_cfg := {"QB": {"core_stats": []}}
	var main_cfg := {"class_rules": {}}
	...
	for team_id in expected_years.keys():
		...
		for year in expected:
			assert_bool(year in actual).is_true()


func test_non_champions_have_zero_count() -> void:
	var world_state := _create_test_world()
	var config := _create_test_config()
	var positions_cfg := {"QB": {"core_stats": []}}
	var main_cfg := {"class_rules": {}}
	...
	for team_id in team_history.keys():
		if not champion_ids.has(team_id):
			...
			assert_bool(years.is_empty()).is_true()
	...
	for i in range(6):
		...
		for j in range(10):
			players.append({
				...
				"college_year": 1,
				"composite_score": 55.0 + i * 3.0 + rng.randf() * 10.0,
				"stats": {},
				"age": 18
			})
