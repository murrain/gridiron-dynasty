## GdUnit4 test suite for H4.1: Franchise Win Totals Accuracy
##
...
func test_franchise_totals_accumulate_correctly() -> void:
	...
	assert_bool(not team_history.is_empty()).is_true()
	...
	for team_id in team_history.keys():
		...
		if all_time_wins > 0 or all_time_losses > 0:
			...
			assert_int(total_games).is_between(30, 40)

	assert_bool(has_wins).is_true()


func test_first_and_last_season_tracking() -> void:
	var world_state := _create_test_world()
	var config := _create_test_config()
	var positions_cfg := {"QB": {"core_stats": []}}
	var main_cfg := {"class_rules": {}}
	...
	for team_id in team_history.keys():
		...
		assert_int(first_season).is_equal(2025)
		assert_int(last_season).is_equal(2027)


func test_totals_match_season_records() -> void:
	var world_state := _create_test_world()
	var config := _create_test_config()
	var positions_cfg := {"QB": {"core_stats": []}}
	var main_cfg := {"class_rules": {}}
	...
	for team_id in expected_totals.keys():
		...
		assert_int(actual_wins).is_equal(expected["wins"])
		assert_int(actual_losses).is_equal(expected["losses"])
	...
	for i in range(4):
		...
		for j in range(10):
			players.append({
				...
				"college_year": 1,
				"composite_score": 60.0 + rng.randf() * 20.0,
				"stats": {},
				"age": 18
			})
