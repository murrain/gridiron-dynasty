## GdUnit4 test suite for G1.1: Game Simulation Determinism
##
...
func test_determine_winner_determinism() -> void:
	var game_matchup := {
		"game_id": "test_game_1",
		"year": 2025,
		"week": 1,
	...
	assert_str(results[0]["winner_id"]).is_equal(results[1]["winner_id"])
	assert_str(results[0]["winner_id"]).is_equal(results[2]["winner_id"])
...
func test_schedule_generation_determinism() -> void:
	var colleges := []
	for i in range(10):
...
func test_full_season_determinism() -> void:
	...
	roster_rng.seed = 11111
	for college in colleges:
		var c: Dictionary = college
		var college_id := String(c["id"])
		rosters[college_id] = {
			"players": [
				{"position": "QB", "composite_score": 70.0 + roster_rng.randf() * 10.0},
				{"position": "RB", "composite_score": 65.0 + roster_rng.randf() * 10.0}
			]
		}
	...
	for i in range(results_run1.size()):
		...
		assert_str(r1["winner_id"]).is_equal(r2["winner_id"])
