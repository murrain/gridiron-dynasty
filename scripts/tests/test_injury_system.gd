...
func run(t) -> void:
	test_config_structure(t)
	test_injury_type_selection(t)
	test_determinism_same_seed(t)
	test_career_ending_injury_forces_retirement(t)
	test_position_multipliers(t)
	test_durability_traits(t)
	test_injury_generation_structure(t)
	test_fallback_with_empty_config(t)
...
func test_config_structure(t) -> void:
	var main_cfg := _make_main_cfg()
	var injury_cfg: Dictionary = main_cfg.get("injury", {}) as Dictionary

	t.assert_true(not injury_cfg.is_empty(), "Injury config exists")
	t.assert_true(injury_cfg.has("base_chance"), "Has base_chance")
	t.assert_true(injury_cfg.has("types"), "Has injury types")
	t.assert_true(injury_cfg.has("position_multipliers"), "Has position multipliers")
	t.assert_true(injury_cfg.has("durability_trait_modifiers"), "Has trait modifiers")

	var types: Array = injury_cfg.get("types", []) as Array
	t.assert_eq(types.size(), 4, "Has 4 injury types in test config")

func test_injury_type_selection(t) -> void:
	var main_cfg := _make_main_cfg()
	...
	t.assert_true(total_injuries > 0, "Injuries occurred during trials")
	t.assert_true(injury_counts.size() > 0, "At least one injury type generated")

func test_determinism_same_seed(t) -> void:
	var main_cfg := _make_main_cfg()
	...
	t.assert_eq(injuries1.size(), injuries2.size(), "Same number of injuries with same seed")
	...
	if injuries1.size() > 0 and injuries2.size() > 0:
		...
		t.assert_eq(inj1.get("type"), inj2.get("type"), "Same injury type with same seed")

func test_career_ending_injury_forces_retirement(t) -> void:
	var main_cfg := _make_main_cfg()
	var positions_cfg := _make_positions_cfg()
	var stats_cfg := _make_stats_cfg()
	...
	t.assert_eq(retired.size(), 1, "Player with career-ending injury retired")

func test_position_multipliers(t) -> void:
	var main_cfg := _make_main_cfg()
	var rng := RandomNumberGenerator.new()
	rng.seed = 11111
	...
	t.assert_true(rb_mult > 1.0, "RB has increased injury risk")
	t.assert_true(k_mult < 1.0, "K has decreased injury risk")
	t.assert_true(rb_mult > k_mult, "RB has higher multiplier than K")

func test_durability_traits(t) -> void:
	var main_cfg := _make_main_cfg()
	var rng := RandomNumberGenerator.new()
	rng.seed = 22222
	...
	t.assert_true(prone_mult > 1.0, "injury_prone increases injury risk")
	t.assert_true(durable_mult < 1.0, "durable decreases injury risk")

func test_injury_generation_structure(t) -> void:
	var main_cfg := _make_main_cfg()
	var injury_cfg: Dictionary = main_cfg.get("injury", {}) as Dictionary
	var rng := RandomNumberGenerator.new()
	rng.seed = 33333
	...
	if injury != null:
		t.assert_true(injury.has("type"), "Has type field")
		t.assert_true(injury.has("severity"), "Has severity field")
		t.assert_true(injury.has("affected_stats"), "Has affected_stats field")
		t.assert_true(injury.has("recovery_timeline"), "Has recovery_timeline field")
		t.assert_true(injury.has("long_term_penalty"), "Has long_term_penalty field")
		t.assert_true(injury.has("career_ending"), "Has career_ending field")

		var timeline: Dictionary = injury.get("recovery_timeline", {}) as Dictionary
		t.assert_true(timeline.has("years_total"), "Timeline has years_total")
		t.assert_true(timeline.has("years_remaining"), "Timeline has years_remaining")
		t.assert_true(timeline.has("status"), "Timeline has status")

		var penalty: Dictionary = injury.get("long_term_penalty", {}) as Dictionary
		t.assert_true(penalty.has("stat_caps"), "Penalty has stat_caps")
		t.assert_true(penalty.has("decline_multipliers"), "Penalty has decline_multipliers")

func test_fallback_with_empty_config(t) -> void:
	var empty_cfg := {}
	var rng := RandomNumberGenerator.new()
	rng.seed = 44444
	...
	t.assert_true(report.has("injured"), "Report has injured field")
	t.assert_true(report.has("roll"), "Report has roll field")
	t.assert_true(report.has("final_chance"), "Report has final_chance field")

	var base := float(report.get("base_chance", 0.0))
	t.assert_approx(base, 0.12, 0.01, "Falls back to default base_chance")
