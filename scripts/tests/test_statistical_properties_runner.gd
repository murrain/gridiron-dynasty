...
func _init() -> void:
	...
	var path = "res://scripts/tests/test_statistical_properties.gd"

	var script = load(path)
	if script == null:
		print("Failed to load test_statistical_properties.gd")
		quit(1)
		return
	...
	if total_failures.is_empty():
		print("test_statistical_properties.gd passed (8 tests)")
		quit(0)
		return
