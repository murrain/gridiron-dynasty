...
func test_collect_free_agents_calculates_demand(helper: TestHelpers) -> void:
	...
	for fa in free_agents:
		var fa_dict := fa as Dictionary
		var player_id := String(fa_dict.get("player_id", ""))
...
func test_generate_team_interest_prioritizes_needs(helper: TestHelpers) -> void:
	...
	# Create FA QB
	var qb := _create_test_player("player-qb", "QB", 26, 78.0)
	var fa_profile := {
		"player_id": "player-qb",
		"position": "QB",
		"age": 26,
	...
	var free_agents := [fa_profile]
	var rng := helper.create_seeded_rng(12345)
	var league_cfg := _load_league_config()

	# Remove QB from one team to create need
	world_state["nfl_rosters"]["CAR"]["players"] = []

	var team_interest := FreeAgency.generate_team_interest(
		world_state,
		year,
		free_agents,
		rng,
...
func test_player_chooses_highest_offer(helper: TestHelpers) -> void:
	...
	var rng := helper.create_seeded_rng(54321)

	var chosen_team := FreeAgency.player_chooses_team(player_id, team_interest, offers, rng)
...
func test_player_chooses_familiarity_bonus(helper: TestHelpers) -> void:
	...
	var rng := helper.create_seeded_rng(99999)
...
func test_franchise_tag_calculates_salary(helper: TestHelpers) -> void:
	...
	var qb_salaries := [25.0, 22.0, 20.0, 18.0, 16.0, 14.0, 12.0]  # Top 5 avg = 20.2

	for i in range(qb_salaries.size()):
		var qb := _create_test_player("qb-%d" % i, "QB", 27, 80.0)
		qb["contract"] = {
			"status": "signed",
	...
	# Add target player
	var target := _create_test_player("player-tag-qb", "QB", 26, 85.0)
	...
	var salary := float(tag.get("salary", 0.0))
	var expected_avg := (25.0 + 22.0 + 20.0 + 18.0 + 16.0) / 5.0  # 20.2
...
func test_franchise_tag_stored_in_world_state(helper: TestHelpers) -> void:
	...
	# Add player to tag
	var player := _create_test_player("player-tag", "EDGE", 25, 82.0)
...
func test_franchise_tag_prevents_free_agency(helper: TestHelpers) -> void:
	...
	# Add player
	var player := _create_test_player("player-tagged", "CB", 26, 84.0)
	player["contract"] = {"status": "expired", "years_remaining": 0}
	...
	var free_agents_before := FreeAgency.collect_free_agents(
		world_state,
		year,
		positions_cfg,
		main_cfg
	...
	var tagged_player := player  # Reference should be same
	var contract_status := String(tagged_player.get("contract", {}).get("status", ""))

	helper.assert_eq(contract_status, "signed", "Tagged player has signed contract")

	var years_remaining := int(tagged_player.get("contract", {}).get("years_remaining", 0))
...
func test_franchise_tag_one_per_team(helper: TestHelpers) -> void:
	...
	# Add two players
	var player1 := _create_test_player("player-1", "WR", 26, 82.0)
	player1["contract"] = {"status": "expired", "years_remaining": 0}

	var player2 := _create_test_player("player-2", "EDGE", 25, 80.0)
	...
	# Tag first player
	var tag1 := FreeAgency.apply_franchise_tag(
		world_state,
		"MIA",
		"player-1",
		"non_exclusive",
	...
	var tag2 := FreeAgency.apply_franchise_tag(
		world_state,
		"MIA",
		"player-2",
		"non_exclusive",
...
func test_franchise_tag_consecutive_penalty(helper: TestHelpers) -> void:
	...
	# Add player
	var player := _create_test_player("player-consecutive", "OL", 27, 78.0)
	player["contract"] = {"status": "expired", "years_remaining": 0}
	...
	for i in range(5):
		var ol := _create_test_player("ol-%d" % i, "OL", 28, 75.0)
	...
	var salary_year_1 := float(tag1.get("salary", 0.0))
	...
	var salary_year_2 := float(tag2.get("salary", 0.0))
...
func test_run_free_agency_signs_players(helper: TestHelpers) -> void:
	...
	# Add free agent
	var player := _create_test_player("player-sign", "TE", 25, 72.0)
	player["contract"] = {"status": "expired", "years_remaining": 0}
...
func test_run_free_agency_deterministic(helper: TestHelpers) -> void:
	...
	# Run 1
	var world_state_1 := _create_test_world()
	...
	var result1 := FreeAgency.run_free_agency(
		world_state_1,
		2024,
		seed,
		positions_cfg,
	...
	var world_state_2 := _create_test_world()
	...
	if result1["signings"].size() > 0:
		var signing1 := result1["signings"][0] as Dictionary
		var signing2 := result2["signings"][0] as Dictionary
