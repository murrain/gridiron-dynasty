...
func _init() -> void:
	...
	# Load configurations
	var configs := ConfigLoader.load_all_configs()
	...
	var seed := 12345
	var world_state := WorldGenerator.generate_world(seed, config, positions_cfg, main_cfg, stats_cfg)

	var college_count := world_state.get("colleges", []).size()
	...
	var year := 2025
	var season_result := CollegeSeason.run(
		world_state,
		year,
		seed,
		config,
	...
	var sample_roster := college_rosters.values()[0] if not college_rosters.is_empty() else {}
